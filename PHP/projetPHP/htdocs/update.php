<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        updateTache($ID_TCH, $nom, $cat, $description, $actif);
        header('Location: ./Main.php');
        exit();
    }
...
<body>
    ...
    <form id="upgradeForm" action="<?php echo $_SERVER['PHP_SELF'] . '?ID_TCH=' . $ID_TCH; ?>" method="POST">
        <label for="nom">Nom :</label>
        <input type="text" id="nom" name="nom" required><br><br>

        <label for="cat2">Catégorie :</label>
        <select id="cat2" name="cat2" required></select><br><br>

        <label for="description">Description :</label>
        <textarea id="description" name="description" required></textarea><br><br>

        <label for="actif">Actif </label>
        <input type="checkbox" id="actif" name="actif"><br><br>
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#nom').val(Tache[0].NOM);
                    $('#cat').val(Tache[0].CAT);
                    $('#description').val(Tache[0].DESCRIPTION);
                    if (Tache[0].ACTIF == 1) {
